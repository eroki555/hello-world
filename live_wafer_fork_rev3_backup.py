...
path = '/Users/021006/Documents/AIT/3_AR/Wafer_thickness'
...
def ref_detector_matchTemplate():
    ...
    # テンプレ画像準備
    template = cv2.imread(path + '/ref_gray_th_200_frame_cut_4.png', 0)
...
while True:
    ...
    resized_frame_cut = cv2.resize(frame_cut, (int((rect_pt2[0]-rect_pt1[0]) / 3), int((rect_pt2[1]-rect_pt1[1]) / 3)))
    # リサイズしたフレームを表示する
    #デバッグ用
    ...
    rect1_pos_start = (int(position_match_local[0]/3),int(position_match_local[1]/3))
    rect1_pos_end = (int((position_match_local[0] + ref_pic_size[0])/3), int((position_match_local[1] + ref_pic_size[1])/3))
    # デバッグ用
    ...
    if len(contours) >= 3 and max_val >200000000:
        time.sleep(0.01)
        if len(contours) >= 3:
            ...
            if a_mid_lower ==0 or b_mid_lower ==0 or a_mid_upper ==0 or b_mid_upper ==0 or a_top_upper ==0 or b_top_upper ==0 or a_btm_upper ==0 or b_btm_upper ==0:
                ...
                # cv2.imshow('th3Cth3', th3Cth3)
                #cv2.imshow('th3Cth3rect', th3Cth3rect)
                resized_th3Cth3rect = cv2.resize(th3Cth3rect, (int(frame_width/4), int(frame_height/4)))
                cv2.imshow('resized_th3Cth3rect', resized_th3Cth3rect)
                cv2.moveWindow('resized_th3Cth3rect', 270, 100)
                cv2.moveWindow('resized_frame_cut', 10, 100)
                cv2.moveWindow('resized_th3', 10, 295)
        else:
            pass
